...
def _normalize_modifier(mod: str) -> str:
    m = mod.strip().lower()
    if m in ("win", "cmd", "command", "super", "meta"):
        return "win"
...
class _PynputHotkeyBackend:
    def __init__(self, modifier: str, key: str, callback: Callable[[], None]):
        ...
        self._target_key_str = key.lower().strip()
    ...
    def _canonical(self, key) -> str:
        if keyboard is not None:
            # 1. Kiểm tra đối tượng phím bổ trợ enum trước (Key.alt, Key.ctrl, Key.shift, Key.cmd...)
        ...
        if "shift" in k_str:
            return "shift"
        if "cmd" in k_str or "win" in k_str:
            return "win"
        return k_str
...
class HotkeyManager:
    ...
    def _create_backend(self):
        ...
        if self._backend_id in {"auto", "win32"} and sys.platform == "win32":
            try:
